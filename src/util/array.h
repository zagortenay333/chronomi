...
Void array_maybe_decrease_capacity (T *a) {
    if ((a->capacity > 4) && (a->count < (safe_mul(a->capacity, 25lu) / 100))) {
        U64 new_cap = a->capacity / 2;
        assert_always(a->count <= new_cap);
...
template <typename T> Array<T> array_new_count (Mem *mem, U64 count)     { Array<T> a; array_init_cap(&a, mem, count); array_ensure_count(&a, count, true); return a; }
...
template <typename T> Elem(T) array_pop         (T *a)            { Auto r = array_get_last(a); a->count--; return r; }
template <typename T> Elem(T) array_pop_or      (T *a, Elem(T) v) { return a->count ? array_pop(a) : v; }
...
Elem(T) *array_push_slot (T *a) {
    if (a->count == a->capacity) {
        U64 new_cap = a->capacity ? static_cast<U64>(1.8 * a->capacity) + 1 : 2;
