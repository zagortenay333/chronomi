...
String  str           (CString s)            { return (String){ .data=const_cast<Char*>(s), .count=(s ? strlen(s) : 0) }; }
...
Bool str_match (String s1, String s2) {
    ...
    return (s1.count == s2.count) && (! strncmp(s1.data, s2.data, s1.count));
}
...
Bool str_to_i64 (CString str, I64 *out, U64 base) {
    errno = 0;
    Char *endptr = 0;
    *out = static_cast<I64>(std::strtol(str, &endptr, base));
