...
//
// The way to deal with custom keys is to either call
// map_init which assumes there are two functions named
// hash and compare or if that doesn't work due to some
// awkward header inclusions call map_init_hc where you
// pass them in manually.
// =============================================================================
#include "util/core.h"
...
template <typename Key, typename Val>
Bool map_has (Map<Key, Val> *map, Key key) {
    return map_get(map, key, (Val*)0);
}
...
template <typename Key, typename Val>
Void map_init_hc (Map<Key, Val> *map, Mem *mem, U64 cap, MapHashFn<Key> hash, MapCmpFn<Key> compare) {
...
Void map_init (Map<Key, Val> *map, Mem *mem, U64 cap) {
    return map_init_hc(map, mem, cap, hash, compare);
}
...
template <typename T>
Void set_add (Set<T> *set, T val) {
    map_add(set, val, static_cast<U8>(0));
}
