...
using Parselet = MarkupAst *(*)(Parser *);
using InfixParselet = MarkupAst *(*)(Parser *, MarkupAst *lhs);
...
static Void parse_blocks (Parser *p, Array<MarkupAst*> *out);
static MarkupAst *parse_filter (Parser *p, U64 lhs_precedence);
static Void parse_inlines (Parser *p, Array<MarkupAst*> *out, Delimiter delim);

U64 markup_ast_hash (MarkupAst *node) {
    return hash(node->tag + node->pos.offset + node->pos.length);
}
...
static Delimiter delim () { return {}; }
static Delimiter delim (TokenTag t1) { return { .t1=t1 }; }
static Delimiter delim (TokenTag t1, TokenTag t2) { return { .t1=t1, .t2=t2 }; }
static Delimiter delim (TokenTag t1, TokenTag t2, TokenTag t3) { return { .t1=t1, .t2=t2, .t3=t3 }; }
...
static MarkupAst *parse_paragraph (Parser *p) {
    ...
    parse_inlines(p, &node->children, delim());
    return complete_node(p, node);
}
...
static MarkupAst *parse_italic (Parser *p) {
    ...
    lex_eat(p->lex);
    parse_inlines(p, &node->children, delim(TOKEN_ASTERISK));
    return complete_node(p, node);
}
...
static MarkupAst *parse_bold (Parser *p) {
    ...
    lex_eat(p->lex);
    lex_eat(p->lex);
    parse_inlines(p, &node->children, delim(TOKEN_ASTERISK, TOKEN_ASTERISK));
    return complete_node(p, node);
}
...
static MarkupAst *parse_highlight (Parser *p) {
    ...
    lex_eat(p->lex);
    lex_eat(p->lex);
    parse_inlines(p, &node->children, delim(TOKEN_ASTERISK, TOKEN_ASTERISK, TOKEN_ASTERISK));
    return complete_node(p, node);
}
...
static MarkupAst *parse_sup (Parser *p) {
    ...
    parse_inlines(p, &node->children, delim(TOKEN_CARET));
    p->stop_parsing_inline_at_newline--;
    return complete_node(p, node);
}
...
static MarkupAst *parse_sub (Parser *p) {
    ...
    lex_eat(p->lex);
    p->stop_parsing_inline_at_newline++;
    parse_inlines(p, &node->children, delim(TOKEN_TILDE));
    p->stop_parsing_inline_at_newline--;
    return complete_node(p, node);
}
...
static MarkupAst *parse_strike (Parser *p) {
    ...
    lex_eat(p->lex);
    lex_eat(p->lex);
    parse_inlines(p, &node->children, delim(TOKEN_TILDE, TOKEN_TILDE));
    return complete_node(p, node);
}
...
static MarkupAst *parse_raw_inline (Parser *p) {
    ...
    reinterpret_cast<MarkupAstRawInline*>(node)->monospace = monospace;

    p->inside_raw_text_region = true;
    Delimiter d = monospace ? delim(TOKEN_BACKTICK, TOKEN_BACKTICK) : delim(TOKEN_BACKTICK);
...
static MarkupAst *parse_link (Parser *p) {
    ...
    n->link = parse_text_until(p, [](Parser *p, Token *token){
        if (token->tag == '>') {
            return true;
        } else if (token->tag == ';') {
            lex_eat(p->lex);
    ...
    n->alias = parse_text_until(p, [](Parser *p, Token *token){
        if (token->tag == '>') {
            lex_eat(p->lex);
            return true;
        } else {
...
static MarkupAst *parse_tag_ref (Parser *p) {
    ...
    if (p->body_tags.mem) set_add(&p->body_tags, markup_ast_get_text(child, p->text));
    return complete_node(p, node);
}
...
static MarkupAstTableCellConfig parse_table_cell_config (Parser *p) {
    ...
    while (true) {
        ...
        if (token->tag == TOKEN_EOF) {
            ...
            config.width = (token->tag == '*') ? -1 : max(1lu, token->u64);
            lex_eat(p->lex);
            lex_eat(p->lex);
            Token *t = lex_eat(p->lex);
            config.height = (t->tag == '*') ? -1 : max(1lu, t->u64);
        } else {
            lex_eat(p->lex);
        }
    }
...
static MarkupAst *parse_table_cell (Parser *p) {
    ...
    lex_eat(p->lex);
    reinterpret_cast<MarkupAstTableCell*>(node)->config = parse_table_cell_config(p);
...
static Bool try_parse_meta_config (Parser *p, MarkupAstMetaConfig *config) {
    ...
    if (token->tag == '@') {
        ...
        if (! (config->flags & MARKUP_AST_META_CONFIG_HAS_TAGS)) map_init(&config->tags, p->mem, 0);
        config->flags |= MARKUP_AST_META_CONFIG_HAS_TAGS;

        U64 end = p->last_eaten_token.offset + p->last_eaten_token.length;
        set_add(&config->tags, str_slice(p->text, start, end-start));
    } else if (token->tag == '#') {
        Token *t = lex_eat(p->lex);
        ...
        config->spoiler = parse_text_until(p, [](Parser *p, Token *token){ return token->tag == ']'; });
    }
...
static MarkupAst *parse_filter_tag (Parser *p) {
    ...
    lex_try_eat(p->lex, TOKEN_WORD);
    complete_node(p, node);
    reinterpret_cast<MarkupAstFilterTag*>(node)->text = markup_ast_get_text(node, p->text);
    return node;
}
...
static MarkupAst *parse_filter_fuzzy (Parser *p) {
    ...
    Auto n = reinterpret_cast<MarkupAstFilterFuzzy*>(node);

    lex_eat(p->lex);

    n->needle = parse_text_until(p, [](Parser *p, Token *token){
...
static MarkupAst *parse_filter_string (Parser *p) {
    ...
    Auto n = reinterpret_cast<MarkupAstFilterString*>(node);
...
static MarkupAst *parse_filter_priority (Parser *p) {
    ...
    if (lex_try_peek(p->lex, TOKEN_U64)) {
        reinterpret_cast<MarkupAstFilterPriority*>(node)->priority = lex_eat(p->lex)->u64;
    }
    return complete_node(p, node);
}
...
static MarkupAst *parse_filter_track (Parser *p) {
    ...
    if (id->tag == TOKEN_U64) {
        reinterpret_cast<MarkupAstFilterTrack*>(node)->id = id->u64;
    } else {
...
static MarkupAst *parse_meta (Parser *p) {
    ...
    reinterpret_cast<MarkupAstMeta*>(node)->config = config;
    ...
    Bool top_level = !p->body_tags.mem;
    if (top_level) map_init(&p->body_tags, p->mem, 0);

    parse_blocks(p, &node->children);

    if (top_level && p->body_tags.count) {
        config->flags |= MARKUP_AST_META_CONFIG_HAS_BODY_TAGS;
        config->body_tags = p->body_tags;
        p->body_tags = {};
    }
...
// it as as the path starting from the input node node.
//
...
Slice<MarkupAst*> markup_ast_path_for_idx (Mem *mem, MarkupAst *node, U64 idx) {
    Auto result = array_new<MarkupAst*>(mem);

    if (node->children.count == 0) return slice(&result);
